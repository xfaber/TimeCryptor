...
namespace TimeCryptor.Classes
{
    public class Contributor
    {
        ...
        public void PublishToBlockchain(VerifyModeEnum vm, Blockchain bc, bool bHonestParty = true)
        {
            ...
            item.round = round;
            item.pp = new PK_T_y_Item() { PK = pp.PK, T = pp.T, y = pp.y };
            item.proof = new Proof_Item[k];
            for (int i = 0; i < k; i++)
            {
                item.proof[i] = new Proof_Item();
                item.proof[i].left = new PK_T_y_ItemExtended() { PK = proof[i].left.PK, T = proof[i].left.T, y = proof[i].left.y };
                item.proof[i].right = new PK_T_y_ItemExtended() { PK = proof[i].right.PK, T = proof[i].right.T, y = proof[i].right.y };
                ...
                if (vm == VerifyModeEnum.NotInteractive)
                {
                    ...
                    {
                        case 0:
                            item.proof[i].left.t = proof[i].left.t;
                            break;
                        case 1:
                            item.proof[i].right.t = proof[i].right.t;
                            break;
                        default:
            ...
            item.contributorName = Name;
            Console.WriteLine($"\n=== Pubblicazione parametri della Parte {Name} sulla blockchain ===");
        ...
        public void SetPublicParams(ulong round, G2 PKLOE, VerifyModeEnum vm)
        {
            ...
            pp = new PK_T_y_Item();
            pp.PK = publicParams.PK;
            pp.T = publicParams.T;
            pp.y = publicParams.y;
            Console.WriteLine($"PK: {pp.PK.ToCompressedPoint()}");
            Console.WriteLine($"T: {pp.T.ToCompressedPoint()}");
            Console.WriteLine($"y: {pp.y.ToString(16)}");

            proof = new Proof_Item[k];
            for (int j = 0; j < k; j++)
            {
        ...
        {
            var array_b_string = "";
            array_b_string += pp.PK.Normalize().ToCompressedPoint().ToLower();
            for (int j = 0; j < k; j++)
            {
                array_b_string += proof[j].left.PK.Normalize().ToCompressedPoint().ToLower() + proof[j].left.T.GetStr(16) + proof[j].left.y;
                array_b_string += proof[j].right.PK.Normalize().ToCompressedPoint().ToLower() + proof[j].right.T.GetStr(16) + proof[j].right.y;
            }
