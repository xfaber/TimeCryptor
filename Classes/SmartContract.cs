...
namespace TimeCryptor.Classes
{
  public class SmartContract
  {
    ...
    public List<string> Verify(VerifyModeEnum vm, ulong round, Blockchain blockChain, GlobalParams globalParams)
    {
      ...
      foreach (var bcRoundItem in bcRoundItemList)
      {
        ...
        int[] rndBitArray;
        if (vm == VerifyModeEnum.Interactive)
        ...
        {
          //controllo (1) - VERIFICA DELLA CHIAVE PK DELLE PROVE --> riferimento a Sezione 3.4.3.3 "Il protocollo TLCS 〖〖Σμ〗^-〗^((k)) interattivo"
          ...
          //controlli (2)(3) --> riferimento a Sezione 3.4.3.3 "Il protocollo TLCS 〖〖Σμ〗^-〗^((k)) interattivo"
      ...
      if (verifiedContributorNameList.Count == 0) throw new Exception("Nessun partecipante onesto! Generazione chiavi compromessa! ");
